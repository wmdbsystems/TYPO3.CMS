...
final class t3lib_div {
	...
	public static function generateRandomBytes($count) {
		...
		if (!isset($bytes{$count - 1})) {
			if (version_compare(PHP_VERSION, '5.3.0', '>=')) {
					// openssl seems to be deadly slow on Windows
				if (TYPO3_OS !== 'WIN' && function_exists('openssl_random_pseudo_bytes')) {
					$isStrong = NULL;
					$bytes = openssl_random_pseudo_bytes(max(4096, $count), $isStrong);
				} else if (function_exists('mcrypt_create_iv')) {
						// Windows php versions below 5.3.4 have a bug when using urandom source (see #16837)
					if (TYPO3_OS !== 'WIN') {
						$randomSource = MCRYPT_DEV_URANDOM;
					} else {
						$randomSource = MCRYPT_RAND;
					}
					$bytes = mcrypt_create_iv(max(4096, $count), $randomSource);
				}
			} else if (TYPO3_OS !== 'WIN' && $fh = @fopen('/dev/urandom', 'rb')) {
				...
				// Fall back if other random byte generation failed until now (on Windows)
			if (!isset($bytes{$count - 1})) {
